...
public class OrderRepositoryImpl implements OrderRepository {
    ...
    public List<Order> getAll() throws SQLException {
        ...
        while (resultSet.next()){
            ArrayList<OrderDetails> orderItems = new ArrayList<>();
            ResultSet resultSet1 = CrudUtil.execute("SELECT * FROM orderDetail WHERE orderId =?",resultSet.getString(1));
            while (resultSet1.next()){
                orderItems.add(new OrderDetails(resultSet1.getString(1),resultSet1.getString(2),resultSet1.getInt(3),resultSet1.getDouble(4)));
            }
